...
class ListageVendeur extends GetView<VendeurController> {
  //
  String? categorie;
  ListageVendeur(this.categorie) {
    print("${categorie!}");
    controller.getParCategorieForAdmin(categorie!);
    //
  }
              ...
              child: GridView.count(
                ...
                children: List.generate(
                  ...
                  (index) => Card(
                    elevation: 0,
                    child: Image.network(
                        "${Utils.url}/produit/image/${controller.images[index]}"),
                  ),
                ),
              ),
            ),
...
/*
  ...
  Future<Widget> getProduitCategorie(String categorie) async {
    ...
    controller.details = RxMap();
    Response rep = await vendeurConnexion.getParCategorieForAdmin(categorie);
    if (rep.statusCode == 200 || rep.statusCode == 201) {
      //print(rep.bodyString!);
      //controller.produits.value = jsonDecode(rep.bodyString!);
      //load.value = true;
      //print("cool___________________");
      return Container(
        child: Obx(
          () => Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Expanded(
                ...
                child: Container(
                  ...
                  child: controller.produits.isNotEmpty
                      ? ListView(
                          controller: ScrollController(),
                          children: List.generate(
                            controller.produits.length,
                            (index) {
                              print(
                                  "____________:::::::::::::${controller.produits[index]['id']}");
                              //
                              //controller.getListeImages(
                              // "${controller.produits[index]['id']}");
                              //
                              return ListTile(
                                ...
                                /* controller.images.isEmpty
                                    ...
                                    : Container(
                                      ...
                                      */
                                title: Text(
                                    "${controller.produits[index]['titreMar']}"),
                                subtitle: Text(
                                    "${controller.produits[index]['prixMar']}  ${controller.produits[index]['deviseMar']}\nStock: ${controller.produits[index]['prixMar']}"),
                              );
                            },
                          ),
                        )
